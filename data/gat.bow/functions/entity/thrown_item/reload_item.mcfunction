...
# わたす
    execute if data storage chuz:context {Reload:1} if entity @s[nbt={Item:{tag:{ChuzData:{ItemID:TechKit_Revo_Reloader}}}}] as @p[tag=Chuz.This] run function minigunbow:item/techkit_revo_reloader/reload/start
    execute if data storage chuz:context {Reload:1} run kill @s

# 最大時
    execute if data storage chuz:context {Reload:0} run function minigunbow:entity/thrown_item/return
